...
do_configure:prepend() {
    ...
    # #include.
    if ! grep -q '#include <fmt/ranges.h>' ${S}/src/rosidlcpp_generator_cpp/rosidlcpp_generator_cpp.cpp; then
        sed -i -e '/^#include <fmt\/format.h>$/a #include <fmt/ranges.h>' \
            ${S}/src/rosidlcpp_generator_cpp/rosidlcpp_generator_cpp.cpp
    fi
}
