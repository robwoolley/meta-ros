...
do_configure:prepend() {
    ...
    # #include.
    if ! grep -q '#include <fmt/ranges.h>' ${S}/src/rosidlcpp_typesupport_fastrtps_c/rosidlcpp_typesupport_fastrtps_c.cpp; then
        sed -i -e '/^#include <fmt\/format.h>$/a #include <fmt/ranges.h>' \
            ${S}/src/rosidlcpp_typesupport_fastrtps_c/rosidlcpp_typesupport_fastrtps_c.cpp
    fi
}
